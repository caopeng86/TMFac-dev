...
class WxPayDataBase
{
	...
	* @param string $value 
	**/
	public function SetSign($form)
	{
		$sign = $this->MakeSign($form);
	...
	**/
	 ...
	 */
	public function MakeSign($form)
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
		$string = $this->ToUrlParams();
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".$form['key'];
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
class WxPayResults extends WxPayDataBase
{
	 ...
	 * 检测签名
	 */
	public function CheckSign($form)
	{
		//fix异常
		if(!$this->IsSignSet()){
	 ...
	 */
	public static function InitFromArray($array, $noCheckSign = false,$form)
	{
		$obj = new self();
		$obj->FromArray($array);
     ...
     * @throws WxPayException
     */
	public static function Init($xml,$form)
	{	
		$obj = new self();
		$obj->FromXml($xml);
		//fix bug 2015-06-29
